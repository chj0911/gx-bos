        ...
        Multiselect.prototype = {
            ...
            events: function() {
                ...
                self.$left.on('dblclick', 'option', function(e) {
                    ...
                    var flag = false;
                    if (checkFlag) {
                        flag = self.checkDataFunc(checkFlag, e);
                    }
                    if(!flag){
            ...
            checkDataFunc: function(checkFlag, e) {
                var self = this;
                if (checkFlag) {
                    var leftSelect = self.$left.find('option:selected');
                    console.log("left"+leftSelect.length);
                    var children = self.$right.find('option');
                    if (children.length >= 1 || leftSelect.length>1) {
                        ModalBox.alert("当前操作只能选择一个选项!")
                        return true;
                    }
                    return false;
                }
