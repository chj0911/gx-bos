...
getZIndex = function()
{
    if( !ZIndex ) ZIndex = 65500; // Modified By Rain Wang
...
getThemePath = (function() {
		    var sc = document.getElementsByTagName('link'), bp = '',
			    i = 0, l = sc.length, re = /macula-ui-.*(?:\.min)?\.css/i;
			
			for( ; i < l; i++ )
			{
			    if( re.test(sc[i].href) )
				{
				    bp = !!document.querySelector ?
					    sc[i].href : sc[i].getAttribute('href',4);
					break;
				}
			}
...
getUrl = (function()
{
    var sc = document.getElementsByTagName('script'), bp = '',
	    i = 0, l = sc.length, re = /jquery\.lhgdialog(?:\.min)?\.js/i;
...
if( (getArgs('t')||'self') === 'self' || top.document.getElementsByTagName('frameset').length > 0 )
    top = window;
...
J.fn.fixie6png = function()
{
	...
	for( var i = 0, l = els.length; i < l; i++ )
	{
	    ...
	    // Modified by Rain Wang	    
		pngPath = getThemePath + bgIMG;
...
J.dialog = function( opts )
{
	...
	ShowCover = function()
	{
		cover = J('#lhgdgCover' + opt.id, doc)[0];
		
		if( !cover )
		{
			var html = '<div id="lhgdgCover' + opt.id + '" style="position:absolute;top:0px;left:0px;' +
	...
	SetDialog = function( dg )
	{
		...
		if( opt.xButton && opt.titleBar )
		    J(xbtnObj).bind( 'click', function(){ (opt.onXclick || S.cancel).call(S); } );
	...
	// Add By Wilson
	S.updateContent = function(content) {
		if( content && typeof(content) != 'boolean')
		{
		    loadObj.style.display = 'none';
			inboxObj.style.display = '';
			if( opt.autoSize ) autoSize();
			$(inboxObj).html(content);
			$(inboxObj).unbind('closeDialog').bind('closeDialog', function(event){
				if(opt.onXclick) {
					opt.onXclick.call(S);
				} else {
					S.cancel();
